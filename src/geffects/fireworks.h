...
// Background sky color (brightness needs to be low, but not too low, backend dependent)
	...
	CRGB sky2(32,32,64);				
#else
	CRGB sky1(0,0,80);				// Background sky color
	CRGB sky2(64,64,128);				// Alternate sky color to create a star twinkle effect 
#endif

	for( uint16_t h = 0; h < NM_MATRIX_WIDTH; h++) {	// All leds will be set to 'sky1' (very dark blue) 
	...
	if( random8() < 48 ) {
		...
		x=random16(NM_MATRIX_WIDTH);
		y=random16(NM_MATRIX_HEIGHT);
		
		Serial.print(x);
		Serial.print(" ");
		Serial.println(y);
		Serial.println(XY2(x,y));
