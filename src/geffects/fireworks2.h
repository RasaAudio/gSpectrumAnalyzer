...
class Dot {
	...
	void Draw() {
		...
		x_pos = map(x_pos, 0, 32, 0, NM_MATRIX_WIDTH-1);	// Scale to Matrix width

		uint16_t y_pos = y >> 10;		// Scaling to get y pixel position 0 - 64
		y_pos = map(y_pos, 0, 64, 0, NM_MATRIX_HEIGHT-1);	// Scale to Matrix height
		...
		if (NM_MATRIX_HEIGHT > 32 && y_pos+1 < NM_MATRIX_HEIGHT && x_pos+1 < NM_MATRIX_WIDTH) {
...
//			hsv2rgb_rainbow( CHSV( random8(), 240, 200), gBurstcolor);    
	...
	void GroundLaunch() {
		...
		if (NM_MATRIX_HEIGHT <= 32) yv = ((20*(NM_MATRIX_HEIGHT+(3*(gGravity*0.8))))-(NM_MATRIX_HEIGHT*5)) + random16(NM_MATRIX_HEIGHT*5);	
		else if (NM_MATRIX_HEIGHT <= 64)  yv = ((14*(NM_MATRIX_HEIGHT+(3*(gGravity*0.8))))-(NM_MATRIX_HEIGHT*5)) + random16(NM_MATRIX_HEIGHT*3);
		else yv = ((10*(NM_MATRIX_HEIGHT+(1.5*(gGravity*0.8))))-(NM_MATRIX_HEIGHT*2)) + random16(NM_MATRIX_HEIGHT*2);
...
#define MAX_SHELLS (NM_MATRIX_HEIGHT/7)
#define MIN_SHELLS (NM_MATRIX_HEIGHT/9)
...
// Background sky color (brightness needs to be low, but not too low, backend dependent)
	...
	CRGB sky2(32,32,64);				
#else
	CRGB sky1(0,0,0x80);				// Background sky color
	CRGB sky2(64,64,128);				// Alternate sky color to create a star twinkle effect 
#endif

	
	for( uint16_t h = 0; h < NM_MATRIX_WIDTH; h++) 
	{	// All leds will be set to 'sky1' (very dark blue) 
		for( int v = 0; v < NM_MATRIX_HEIGHT; v++) 
		{
	...
	if( random8() < 48 ) 
	{
		...
		/*
		int x=random16(NM_MATRIX_WIDTH);
		int y=random16(NM_MATRIX_WIDTH);
		int off=XY2(random16(NM_MATRIX_WIDTH),random16(NM_MATRIX_HEIGHT));
		...
		*/
		...
		matrixleds[XY2(random16(NM_MATRIX_WIDTH),random16(NM_MATRIX_HEIGHT))] = sky2;	
	}
	
	static uint8_t launchcountdown = 0;
	static uint16_t re_launchcountdown = 0;
     
	if(gDot[MAX_SHELLS-1].show == 0) 
	{		// If the last shell has exploded
	...
	{		// Always moves and draws the MAX number of sparks not the actual number of sparks (Wasteful!?)
